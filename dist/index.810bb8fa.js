...
const createPositiveRandomNumber = function() {
    return Math.trunc(Math.random() * (state.maxNum - state.minNum) + 1) + state.minNum;
};
const checkMaxGreater = function(negativeVal = false) {
    ...
    const condition = negativeVal ? state.minNum < state.maxNum : state.minNum > state.maxNum;
    if (condition) {
        messageError.textContent = "Minimum number can't be higher than maximum number. Random number created between 1-20.";
        createSetTimeout();
        return false;
    }
};
...
const checkGuessNumber = function() {
    ...
    if (state.maxNum > 0 && state.minNum > 0 || state.maxNum > 0 && state.minNum < 0) {
        if (val > state.maxNum || val < state.minNum) {
            message.textContent = `Number range is ${state.minNum}-${state.maxNum}.`;
            return;
        }
...
const checkMinandMaxFields = function() {
    ...
    if (!isFinite(minNumVal) || !isFinite(maxNumVal)) {
        messageError.textContent = 'Only numbers as input are valid. Random number created between 1-20.';
        clearInputFields();
        createSetTimeout();
        setValuesToDefault();
        return;
    ...
    if (maxNumVal === '' || minNumVal === '') {
        minNum.value === '' ? minNum.style.border = '2px solid #C02B0B' : maxNum.style.border = '2px solid #C02B0B';
        // Set back to 20 and 1 and clear input fields
        messageError.textContent = 'Please fill in both fields. Random number created between 1-20.';
        clearInputFields();
        createSetTimeout();
        setValuesToDefault();
        return;
...
const init = function() {
    ...
    if (state.maxNum === state.minNum) {
        messageError.textContent = "Numbers can't be the same. Random number created between 1-20.";
        createSetTimeout();
        clearInputFields();
        setValuesToDefault();
        return;
    ...
    if (state.maxNum - state.minNum === 1 || state.maxNum - state.minNum === -1) {
        ...
        messageError.textContent = 'Please use wider range. Random number created between 1-20.';
        createSetTimeout();
        clearInputFields();
        setValuesToDefault();
        return;
    ...
    if (state.maxNum < 0 && state.minNum < 0) {
        ...
        state.secretNum = Math.trunc(Math.random() * (state.maxNum - state.minNum) - 1) + state.minNum;
        setValuesToNewGame();
        clearInputFields();
        return;
    }
    ...
    if (state.maxNum < 0 || state.minNum < 0) {
        ...
        if (num <= Math.abs(state.minNum)) num *= Math.random() > 0.5 ? 1 : -1;
        state.secretNum = num;
        setValuesToNewGame();
        clearInputFields();
    }
...
elements.forEach((element)=>element.addEventListener('keypress', function(e) {
        state.playing = true;
